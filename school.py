...
# ── 날짜 계산: 오후 1시 이후면 내일 기준, 자정~오전은 오늘 기준 ──
now        = datetime.now()
# 오후 13시 이후 → 내일 데이터
...
if weekday_num >= 5:
    ...
    if meal_err:
        ...
        for row in meal_rows:
            ...
            extra = ""
            if kcal:
                extra += f'<span style="background:#fff3cd; color:#856404; border-radius:12px; padding:2px 10px; font-size:.82rem; margin-right:6px;">🔥 {kcal}</span>'
...
has_classroom = "교실" in student_tt  # 교실 정보 있는지
...
def subject_badge(subj: str, room: str = "") -> str:
    if not subj:
        return '<span style="color:#ccc; font-size:.85rem;">—</span>'
    c = subject_colors.get(subj, "#74b9ff")
    room_tag = f'<span style="font-size:.72rem; color:#888; margin-left:4px;">📍{room}</span>' if room else ""
    return (
        f'<span style="background:{c}22; color:{c}; border:1.5px solid {c}; '
        f'border-radius:16px; padding:3px 12px; font-weight:700; font-size:.9rem;">'
        f'{subj}</span>{room_tag}'
    )

def render_day_card(day: str, subjects: list, rooms: list, highlight: bool = False) -> str:
